...
import { insertNewLamb, getAllLambs } from '../databases/DataStore';
...
const AddNewScreen = props => {
    ...
    const handleLambingConfirm = (date) => {
        ...
        console.log("prev date: " + getRandomMatingDate(date).toDateString());
        setLambingDate(date);
    ...
    const onSubmit = () => {
        //console.log(dameID.value);
        const cardi = new CardDetails(id.value, dameID.value, sireID.value, lambingDate, matingDate, "male", birthWeight.value, remarks.value);
        console.log(cardi.toJSON());
        insertNewLamb(cardi.id, cardi.toJSON()).then(() => {
            console.log("added new lamb");
    ...
    const getSubmitButton = () => {
        const showSubmitButton = idIsSubmittable && dameIDIsSubmittable && sireIDIsSubmittable && remarksIsSubmittable && birthWeightIsSubmittable;
        if (showSubmitButton)
            return (<Button title="Submit" onPress={onSubmit} containerStyle={{ alignSelf: 'center' }} buttonStyle={{ backgroundColor: Colors.tertiary }} />);
        else
            return (<View></View>);
    }
